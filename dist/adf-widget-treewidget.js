...
angular.module('adf.widget.treewidget').service('data', ['$http', '$q', 'bus', 'config', function($http, $q, bus, config) {
    ...
    var fetchJsonData = function() {
        if (typeof(jsonData) !== 'undefined') {
            ...
            return $http.get('./src/data.json').success(function(data) {
                setJsonData(data);
                return data;
            })
        } else {
...
d3 = function() {
        ...
        function dragstart(id, position, move, end) {
            return function() {
                var target = this,
                    ...
                    dragged = 0,
                    offset, w = d3.select(d3_window).on(move + "." + drag, moved).on(end + "." + drag, ended),
    ...
    d3.behavior.zoom = function() {
        var translate = [0, 0],
            translate0, scale = 1,
            distance0, scale0, scaleExtent = d3_behavior_zoomInfinity,
            mousedown = "mousedown.zoom",
            mousemove = "mousemove.zoom",
            mouseup = "mouseup.zoom",
            event = d3_eventDispatch(zoom, "zoom"),
            x0, x1, y0, y1, touchtime;
        ...
        function touchstarted() {
            var target = this,
                ...
                w = d3.select(d3_window).on(touchmove, moved).on(touchend, ended).on(mousedown, null).on(mousemove, null),
                dragRestore = d3_event_dragSuppress();
            scale0 = scale;
            translate0 = {};
            distance0 = 0;
    ...
    d3.geom.voronoi = function(points) {
        ...
        function voronoi(data) {
            ...
            if (clipPolygon)
                for (i = 0; i < n; ++i) clipPolygon.clip(polygons[i]);
